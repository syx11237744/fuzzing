...
![libexpat — driver 源码](assets/libexpat/driver.png)

![libexpat — 12h 覆盖率趋势](assets/libexpat/coverage.png)

![libexpat — scan-build HTML 报告](assets/libexpat/scan-build.png)
...
| 指标 | 数值 |
|---|---|
| 累计执行 | **1,110,394,084**（11 亿） |
| 平均 exec/s | **25,650** |
| 最终 cov | **545 / 1,466 PCs (37.2%)** |
| 最终 ft (features) | 3,302 |
| Worker 重启次数 | 413 |
| 崩溃 artifact 数 | **226** |
| **dedup 后唯一根因数** | **1** |

**226 崩溃 artifact，dedup 后只剩 1 个独特根因**——`libFuzzer: fuzz target exited @ yy_fatal_error scanner.c:2463`。这是 fork 模式 + dedup 价值的最直观例证：单进程模式下 12 分钟就要中断，fork 模式跑满 12h 但产生大量同源崩溃文件，必须 dedup 才能看清"是 1 个 bug 不是 226 个"。
...
![libconfig — driver 源码](assets/libconfig/driver.png)

![libconfig — 12h 覆盖率趋势](assets/libconfig/coverage.png)

![libconfig — scan-build 无告警 / dedup 输出](assets/libconfig/scan-build.png)
...
| 指标 | 数值 |
|---|---|
| 累计执行 | **231,062,682**（2.3 亿） |
| 平均 exec/s | **5,335**（被 OOM/慢输入拖累，仅为 libconfig 的 1/5） |
| 最终 cov | **1,350 / 3,950 PCs (34.2%)** |
| 最终 ft (features) | 9,248 |
| Worker 重启次数 | 344 |
| 崩溃 artifact 数 | **20** |
| OOM artifact 数 | **102** |
| Slow-unit artifact 数 | **6** |
| **dedup 后唯一根因数** | **3** |
...
| # | 触发次数 | SUMMARY |
|---|---|---|
| 1 | 11 | `heap-buffer-overflow ucl_hash.c:266 in ucl_hash_destroy` **（12h 新发现）** |
| 2 | 6 | `heap-buffer-overflow ucl_util.c:2192 in ucl_strnstr`（烟雾测试已发现） |
| 3 | 3 | `heap-buffer-overflow ucl_util.c:2195 in ucl_strnstr`（同函数另一行，同 off-by-one 的另一条路径） |

**新发现 ① `ucl_hash_destroy` OOB**（11/20 个 artifact）——递归 cleanup 路径：

```
ucl_hash_destroy → ucl_object_free_internal → ucl_object_dtor_unref_single → ucl_hash_destroy → ...
```

栈深度 30+ 层，触发是嵌套对象树析构时遍历到一个状态损坏的内部 hash 桶。这条路径只在解析后**销毁**对象时执行，正好被我们 driver 末尾的 `ucl_object_unref(root)` 触发。

**已知发现 ② `ucl_strnstr` off-by-one**（9/20 个）——BSD-style 子串查找：

```c
do {
    if ((sc = *s++) == 0 || len-- < mlen)  /* OOB: s++ 已越过 buffer */
        return NULL;
} while (sc != c);
```

`len < mlen` 检查在 `*s++` **之后**——deref 已经越界。

**OOM 102 个 artifact**——抽样栈全部指向：

```
ucl_object_copy_internal ucl_util.c:3650/3702 (递归 deep copy)
```

可能是 `.include` 宏 / 变量展开内部对象树拷贝失控，small input → GB 级内存放大。**强 DoS 信号**。

**Slow-unit 6 个**——fork 模式持续 CPU 占用下找到，与 libexpat 那 4 个"系统抖动假阳"不同，这些可能是真的算法复杂度问题，但未深入分析。
...
![libucl — driver 源码](assets/libucl/driver.png)

![libucl — 12h 覆盖率趋势](assets/libucl/coverage.png)

![libucl — scan-build HTML 报告（11 告警）](assets/libucl/scan-build.png)

\newpage

## 5. 综合结论

| 项目 | 12h fuzz 累计执行 | 最终 cov | crash 唯一根因 | OOM | SA 告警 |
|---|---|---|---|---|---|
| libexpat | 245 M | 4,480 (55.5%) | **0** | 0 | 3（全误报） |
| libconfig | **1,110 M** | 545 (37.2%) | **1**（DoS 设计缺陷） | 0 | 0 |
| libucl | 231 M | 1,350 (34.2%) | **3**（含 1 个新发现） | **102** | 11 |

**方法学层面的发现**：

1. **静态分析与动态测试强烈互补**——libconfig 上 SA 报 0 而 fuzz 找到 DoS；libucl 上 SA 报 11 个但 12h fuzz 撞到的 3 个 ASan crash **没有一个被 SA 预先标出**。两个手段覆盖完全不同的 bug 类别（局部模式 vs 路径触发）。
2. **Dedup 不可或缺**——libconfig 226 个崩溃 artifact 实为 1 个根因，libucl 20 个为 3 个。无 dedup 就是"假繁荣"。
3. **OOM 是被低估的发现类型**——libucl 撞出 102 个 OOM 但只有 20 个 ASan crash。强 DoS 信号往往以 OOM 形式呈现，需作为一等公民对待。
4. **"bug 与否"取决于安全模型**——libucl 的 trusted-input 政策让所有内存安全发现"合规存在"。fuzz 报告必须直面：找到 bug 不等于上游会接受。
5. **Fork 模式是 fuzz 12h 的必备配置**——若仍用单进程，libconfig 第 15 分钟、libucl 第 30 秒就要中断；fork 模式把这两种情况都跑满了 12h。
